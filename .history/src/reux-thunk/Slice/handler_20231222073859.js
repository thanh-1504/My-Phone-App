...
export const handleGetData = createAsyncThunk("getData", async () => {
  const response = await getDataMainPage();
  const data = response.filter((item) => item.mainPage);
  return data;
});
...
export const handleGetDataMainPage = createAsyncThunk(
  "getDataMainPage",
  async () => {
    const response = await getDataMainPage();
    const data = response.filter((item) => item.mainPage);
