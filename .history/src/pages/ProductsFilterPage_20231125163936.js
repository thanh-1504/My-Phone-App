...
const ProductsFilterPage = () => {
  ...
  const { allData, dataFilter } = useSelector((state) => state.nav);
  ...
  const { nameBrand, brandFilter } = dataFilter;
  console.log(brandFilter);
  if (!nameBrand || !brandFilter) return;
  const data = brandFilter.filter((item) => item.slugFilter === slug);
  console.log(data[0].dataBrand);
  return (
    <LayoutPage>
      <Header></Header>
