...
const schema = yup.object({
  ...
  phoneNumber: yup.number().min(10).required("Vui lòng nhập số điện thoại"),
});
const PaymentPage = ({ data }) => {
  ...
  if (errors.phoneNumber?.message && !errors.name && ) toast.error(errors?.phoneNumber?.message);
  if (errors.email?.message && !errors?.name)
