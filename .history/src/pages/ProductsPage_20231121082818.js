...
import { handleGetData, handleGetDataBySlug } from "../reux-thunk/Slice/handler";
import { getDataBySlug } from "../reux-thunk/Slice/request";
...
const ProductsPage = () => {
  ...
  useEffect(() => {
    dispatch(handleGetDataBySlug)
  }, []);

  useEffect(() => {
    async function test() {
      const data = await getDataBySlug("iPhone");
      console.log(data);
    }
    test();
  });
  ...
  //       return (
  //         <>
  //           {dataMacProduct.length > 0 &&
  //             dataMacProduct.map((dataItem) => (
  //               <ProductsItem data={dataItem}></ProductsItem>
  //             ))}
  //         </>
  //       );
  ...
  //       return (
  //         <>
  //           {dataAirpodProduct.length > 0 &&
  //             dataAirpodProduct.map((dataItem) => (
  //               <ProductsItem data={dataItem}></ProductsItem>
  //             ))}
  //         </>
  //       );
  ...
  //       return (
  //         <>
  //           {dataWatchProduct.length > 0 &&
  //             dataWatchProduct.map((dataItem) => (
  //               <ProductsItem data={dataItem}></ProductsItem>
  //             ))}
  //         </>
  //       );
