...
const PaymentPage = ({ data, isClose = false }) => {
  const { dataPayment } = useSelector((state) => state.option);
  return (
    <div>
      <div className={`z-50 modal bg-black/40 ${isClose && "block"}`}>
        <div className="modal-content w-[895px] h-[450px]  overflow-y-auto overflow-x-hidden bl">
