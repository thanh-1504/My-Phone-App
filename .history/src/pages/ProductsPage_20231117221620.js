...
const ProductsPage = () => {
  ...
  if (!dataIp || !dataiPhoneProduct || !dataiPadProduct) return;
  const dataIphone = dataiPhoneProduct[0].sectionIphone;
  const dataIpad = dataiPadProduct[1].sectionIpad;
  const getProduct = () => {
    switch (slug) {
      case "iPhone": {
        return (
          <>
            {dataIphone.map((dataItem) => (
