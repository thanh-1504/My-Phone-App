...
const ProductsItem = ({ style = "", data, ...props }) => {
  ...
  if (!data) return;
  const { name, price, imgSrc, imgMainPage } = data;
  const modelProduct = data.name.split(" ").join("");
  console.log(imgMainPage);
  return (
    <div className="mb:w-full mb:px-2 sm:w-full sm:px-3 md:h-[480px] lg:px-4 lg:h-auto">
