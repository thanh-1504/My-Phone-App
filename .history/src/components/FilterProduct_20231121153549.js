...
const FilterProduct = ({ slug = "" }) => {
  ...
  switch (slug) {
    ...
    case "Watch": {
      style =
        "text-sm font-medium text-white mb:py-[7px] mb:px-[30px] mb:mr-4 mb:mb-3 mb:tracking-[2px] cursor-pointer transition-all max-w-[180px] lg:pr-10";
      break;
    }
    default:
      break;
  ...
  return (
    <div className="filter-wrap ">
      <div className=" mb:flex mb:whitespace-nowrap mb:overflow-x-auto lg:justify-center lg:mt-10 items-center">
        ...
        {data.map((item) => (
          <Link className={`${style}`}>{item.name}</Link>
        ))}
      </div>
    </div>
  );
