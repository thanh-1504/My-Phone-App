...
import { handlePayment } from "../../reux-thunk/Slice/paymentSlice";
const PaymentInfoProduct = ({ data = {} }) => {
  ...
  console.log(numberOrder);
  let price = (data?.dataPayment?.price).slice(0, 10);
  let price2 = data?.dataPayment?.price;
  console.log(price.charAt(2).replace(""));
  return (
      ...
      <div className="w-full h-10 bg-[#f9f9f9]">
        <div className="leading-[2.5rem] float-right mr-[118px]">
          ...
          <span className="text-base font-semibold text-red-500 select-none">
            {+data?.dataPayment?.price * numberOrder}
            {price}
          </span>
        </div>
      </div>
