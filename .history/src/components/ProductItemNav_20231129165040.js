...
const ProductItemNav = ({ data, ...props }) => {
  ...
  const { name, price, imgSrc, typeProduct,slug } = data;
  const modelProduct = data.name.split(" ").join("");
  console.log(typeProslugduct);
  return (
    <div
    ...
    >
      <div className="flex justify-center">
        <ImageItem
          onClick={() =>
            navigate(`/products/detail/${modelProduct}/${data.id}`)
          }
          srcImg={imgSrc}
        ></ImageItem>
      </div>
      <div
        onClick={() => navigate(`/products/detail/${modelProduct}/${data.id}`)}
