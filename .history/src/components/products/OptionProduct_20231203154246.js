...
const OptionProduct = ({ data }) => {
  ...
  if (!data) return;
  // lg:mb-7 mb:mb-8 mb:mr-2 sm:mb-0
  return (
    <>
          ...
          data[0]?.optionProduct.map((item, index) => (
            <>
              ...
              <label
                ...
                className={`bg-[#2f3033] ${ data[0]?.optionProduct?.length > 3 ? "mb:mb-10" : ""} mb:p-2 lg:my-2 lg:px-3 rounded-md lg:text-sm cursor-pointer border product--checked${
                  index + 1
      ...
      {data[0]?.color && (
        <p className={`text-white mb:mb-4  lg:mt-0 ${data[0]?.optionProduct?.length > 3 ? 'lg:mt-[-10px] mb:mt-1' : 'mb:mt-8'}`}>Màu sắc</p>
      )}
      <div>
        {data[0]?.color &&
