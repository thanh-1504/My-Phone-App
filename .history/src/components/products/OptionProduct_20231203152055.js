...
const OptionProduct = ({ data }) => {
  ...
  return (
    <>
      ...
      >
        {data[0]?.optionProduct &&
          data[0]?.optionProduct.map((item, index) => (
            <span key={v4()} className="lg:mb-7 mb:mb-8 mb:mr-2 sm:mb-0">
              <input
                name="capicity"
              ...
              <label
                ...
                className={`bg-[#2f3033] mb:p-2 lg:my-2 lg:px-3 rounded-md lg:text-sm cursor-pointer border product--checked${
                  index + 1
                } mb:mr-3 lg:mr-4 outline-none border-none}`}
              >
      ...
      {data[0]?.color && (
        <p className={`text-white mb:mb-4  lg:mt-0 ${data[0]?.optionProduct?.length > 3 ? 'lg:mt-[-20px] mb:mt-1' : 'mb:mt-8'}`}>Màu sắc</p>
      )}
      <div>
        {data[0]?.color &&
