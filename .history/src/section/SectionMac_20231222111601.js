...
const SectionMac = () => {
  ...
  const data = dataMac.slice(14, 21);
  const width = window.innerWidth;
  useEffect(() => {
    // dispatch(handleGetData());
    dispatch(handleGetData({ type: "", page: "mainPage" }));

  }, [dispatch]);
  if (!dataMac) return;
  console.log(dataMac);
  return (
    <section className="Mac md:px-9 lg:pr-0 sm:px-6 lg:px-0">
      <Brand className="mb-[60px]" nameBrand="Mac"></Brand>
      <LayoutProduct>
        <Swiper
