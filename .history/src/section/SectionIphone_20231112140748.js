...
import { addData } from "../reux-thunk/Slice/request";
import { dataIphone } from "../constant";
import { handleGetDataIphone } from "../reux-thunk/Slice/globalSlice";
const test = [{}, {}, {}, {}, {}];
const SectionIphone = () => {
  const dispatch = useDispatch();
  const width = window.innerWidth;
  // const { dataIp } = useSelector((state) => state.global);
  const a = useSelector((state) => state.global);
  console.log(a);
  // useEffect(() => {
  //   console.log(dispatch(handleGetDataIphone()));
  // });
